...
{{ list_search_results length="5" order="bypublishdate desc" constraints="type is prueba" }}
<div class="post">
<h2 class="post-title"><a href="{{ uri options="article" }}" rel="bookmark" title="{{ $gimme->article->name }}"</a></h2>
<p class="post-details">Publicado en {{ $gimme->article->publish_date|camp_date_format:"%e %M %Y" }} by {{ $gimme->article->author->name }} in <a href="{{ uri options="section" }}">{{ $gimme->section->name }}</a></p>
...
<div class="postinformation">No se han encontrado resultados</div>
{{ /if }}
